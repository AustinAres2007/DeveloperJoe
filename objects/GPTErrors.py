from . import GPTConfig as _gptconfig
...
class ConversationErrors:
    ...
    NO_CONVO = f"You either do not have a conversation with {_gptconfig.BOT_NAME}, or the provided name does not match any chats you currently have."
    HAS_CONVO = "There is already a conversation with the specified name."
    CANNOT_CONVO = f"""You cannot interact with {_gptconfig.BOT_NAME} inside this channel. You may only interact with {_gptconfig.BOT_NAME} in 
    a server text channel, A private server thread (Only two users, you and {_gptconfig.BOT_NAME}) or in Direct Messages."""
    NO_CONVO_WITH_NAME = "No conversation with the specified name."
    CONVO_LIMIT = f"You cannot start anymore than {_gptconfig.CHATS_LIMIT} chats."
    CONVO_NEEDED_NAME = "If you have any more than 1 chat, you must chose a name."
    ALREADY_PROCESSING_CONVO = f"{_gptconfig.BOT_NAME} is already processing a request for you."
