...
async def _gpt_ask_base(query: str, context: GPTConversationContext | None,  model: str, api_key: str, model_configuration: usermodelhandler.CustomisationProfile | None=None, **kwargs) -> responses.OpenAIQueryResponse:
    ...
    try:
        async with openai.AsyncOpenAI(api_key=api_key, timeout=developerconfig.GPT_REQUEST_TIMEOUT) as async_openai_client:
            _reply = await async_openai_client.chat.completions.create(model=model, messages=temp_context, **kwargs | model_configuration.raw if model_configuration else {})
...
async def _gpt_ask_stream_base(
    ...
    model: str, 
    api_key: str, 
    model_configuration: usermodelhandler.CustomisationProfile | None=None,
    ...
    try:
        ...
        try:
            async with openai.AsyncOpenAI(api_key=api_key) as async_openai_client:
                _reply = await async_openai_client.chat.completions.create(messages=history, model=model, stream=True, **kwargs | model_configuration.raw if model_configuration else {})
...
def register_model(cls: Type[AIModel]) -> Any:
    try:
        if len(registered_models) == 25:
            print("Cannot register more than 25 models due to a discord limitation.")
            exit(1)
...
class AIModel(ABC):
    ...
    def __init__(self, member: discord.Member, customisation_profile: usermodelhandler.CustomisationProfile | None=None) -> None: # TODO: Change so that the customised model instance is provided, not its name.
        ...
        self._custom_args_set = customisation_profile
        self._context: ReadableContext = ReadableContext()
        self._image_reader_context: ReaderContext | None = None
        self.member = member
        
        self.display_name += f" ({self.configuration_profile.model_name})" if self.configuration_profile else ""
        
        if not self._check_user_permissions():
            raise exceptions.ModelError(missing_perms)
        
        if isinstance(customisation_profile, usermodelhandler.CustomisationProfile):
            customisation_profile.check_items_with(self.accepted_keys)
    ...
    @property
    def configuration_profile(self) -> usermodelhandler.CustomisationProfile | None:
        return self._custom_args_set
