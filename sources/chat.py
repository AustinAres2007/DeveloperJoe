...
class DGChat:
    ...
    def __init__(self,
        ...
        custom_model_configuration_instance = usermodelhandler.get_user_model(self.member, str(custom_model_configuration)) if custom_model_configuration else None
        if isinstance(model, type(models.AIModel)):
            self.model: models.AIModel = model(member, custom_model_configuration_instance) # type: ignore shutup I did the check
        else:
            self.model: models.AIModel = commands_utils.get_modeltype_from_name(str(model))(member, custom_model_configuration_instance)
