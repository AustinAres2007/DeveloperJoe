...
class gpt(commands.Cog):
    ...
    async def start(self, interaction: discord.Interaction, name: str="0", stream_conversation: Union[str, None]=None):
        try:
            ...
            elif chats != 0 and name == "0":
                return await interaction.response.send_message(GPTErrors.ConversationErrors.CONVO_NEEDED_NAME, ephemeral=False) 
            ...
            self.client.add_conversation(interaction.user.id, name, convo)
    ...
    async def ask(self, interaction: discord.Interaction, message: str, name: str="0", stream: Union[str, None]=None):
        try:
            if interaction.channel and interaction.channel.type in [discord.ChannelType.private_thread, discord.ChannelType.text, discord.ChannelType.private, discord.TextChannel]:
                if isinstance(convo := self.client.get_user_conversation(interaction.user.id, name), GPTChat.GPTChat):
                    ...
                    return await interaction.followup.send(f"## {name if name != '0' else 'Conversation'}\n\n", ephemeral=False)
    ...
    async def get_info(self, interaction: discord.Interaction):
        if isinstance(convo := self.client.get_user_conversation(interaction.user.id), GPTChat.GPTChat) and self.client.application:
