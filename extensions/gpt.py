...
class gpt(commands.Cog):
    ...
    @discord.app_commands.describe(name="The name of the chat you will start (Will be used when saving the transcript)", stream="""
                                   If you want DeveloperJoe's reply to be streamed (Reply will appear as it's generated by the server. Default: No)""")
    ...
    async def start(self, interaction: discord.Interaction, name: str="", stream: str="False"):
        ...
        if stream not in ["True", "False"]:
            return await interaction.followup.send("Incorrect save setting.")
        if (stream == "True" and type(interaction.channel) not in [discord.Thread, abc.GuildChannel, discord.PartialMessageable, discord.TextChannel]) or interaction.channel == None:
